...
<div class="space-y-6 max-w-7xl mx-auto">
        ...
        <div class="grid grid-cols-3 gap-6 items-start">
            ...
            <div class="col-span-2 space-y-6">
                ...
                <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                    <h3 class="font-bold text-gray-900 text-sm mb-4">Grafik Timeline Analisis Insiden ({{ $targetFrames ?? 32 }} Frame)</h3>
                ...
                <!-- Galeri Frame Kejadian (Top 3 Frame) -->
                <div class="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-2.5">
                    <form action="{{ route('user.verifikasi', $analysis->id) }}" method="POST">
                        @csrf
                    ...
                    <a href="{{ route('user.riwayat') }}" class="w-full bg-white border border-emerald-700 text-emerald-700 py-3 rounded-xl text-xs font-semibold hover:bg-emerald-50 transition flex items-center justify-center gap-2 block text-center shadow-sm">
                        <i class="fa-solid fa-clock-rotate-left"></i> Lihat Riwayat Analisis
                    </a>
                </div>
            </div>
        </div>
    @else
        ...
        <div class="bg-white border border-gray-200 rounded-2xl p-16 text-center space-y-4 shadow-sm">
            ...
            <div class="pt-4">
                <a href="{{ route('user.beranda') }}" class="inline-flex items-center gap-2 bg-emerald-700 text-white px-6 py-3 rounded-xl text-xs font-semibold hover:bg-emerald-800 transition shadow-sm">
...
@if(isset($analysis))
<!-- Chart.js Script Dinamis Sesuai Target Frame -->
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const ctx = document.getElementById('timelineChart').getContext('2d');

        // Ambil nilai target frame dari Controller/Session
        const totalFrames = {{ $targetFrames ?? 32 }};
            ...
            options: {
                ...
                scales: {
                    y: {
                        beginAtZero: true,
                        max: 100,
                        title: { display: true, text: 'Bobot Attention (%)', font: { size: 10 } }
                    },
                    x: {
                        ticks: {
                            autoSkip: false,
                        ...
                        title: { display: true, text: 'Frame (Total ' + totalFrames + ' Frame)', font: { size: 10 } }
                    }
                }
            }
        });
