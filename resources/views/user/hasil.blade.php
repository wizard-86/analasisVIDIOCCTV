...
<div class="space-y-6 max-w-7xl mx-auto">
    ...
    @if(isset($analysis))
        <!-- KONDISI 1: JIKA SUDAH ADA DATA / VIDEO DIANALISIS -->
        <div class="grid grid-cols-3 gap-6 items-start">
            <!-- Kolom Kiri: Galeri Frame & Distribusi (Span 2) -->
            ...
            <div class="space-y-6">
                ...
                <div class="space-y-2.5">
                    <form action="{{ route('user.abaikan') }}" method="GET">
                        <button type="submit" class="w-full bg-white border border-gray-200 text-gray-700 py-3 rounded-xl text-xs font-semibold hover:bg-gray-50 transition flex items-center justify-center gap-2 shadow-sm">
                            <i class="fa-solid fa-ban"></i> Abaikan / Salah Deteksi
                        </button>
                    </form>

                    <a href="{{ route('user.analisis.selesai') }}" class="w-full bg-emerald-700 text-white py-3 rounded-xl text-xs font-semibold hover:bg-emerald-800 transition flex items-center justify-center gap-2 block text-center shadow-sm">
                        <i class="fa-solid fa-floppy-disk"></i> Simpan Hasil Analisis
                    </a>

                    <a href="#" target="_blank" class="w-full bg-white border border-emerald-700 text-emerald-700 py-3 rounded-xl text-xs font-semibold hover:bg-emerald-50 transition flex items-center justify-center gap-2 block text-center shadow-sm">
    ...
    @else
        <!-- KONDISI 2: EMPTY STATE (JIKA BELUM ADA VIDEO / DATA KOSONG) -->
        <div class="bg-white border border-gray-200 rounded-2xl p-16 text-center space-y-4 shadow-sm">
            ...
            <p class="text-xs text-gray-400 max-w-md mx-auto leading-relaxed">
                Anda belum mengunggah rekaman video keamanan untuk diproses oleh kecerdasan buatan. Silakan unggah video melalui menu di bawah untuk melihat rincian frame dan deteksi anomali.
            </p>
            <div class="pt-4">
                <a href="{{ route('user.upload') }}" class="inline-flex items-center gap-2 bg-emerald-700 text-white px-6 py-3 rounded-xl text-xs font-semibold hover:bg-emerald-800 transition shadow-sm">
                    <i class="fa-solid fa-cloud-arrow-up"></i> Unggah Video Sekarang
                </a>
            </div>
        </div>
    @endif
